...
Pour une variable $X_j$ et une composante discriminante $f_h$, la corrélation pondérée est définie par :
$$ r_{jh} =  \frac{ \langle X_j , f_h \rangle_W }{ \|X_j\|_W \, \|f_h\|_W } $$

où $\langle \cdot, \cdot \rangle_W$ désigne le produit scalaire pondéré par la matrice $W$, et $\Vert \cdot \Vert_W$ la norme associée. 

Sous forme matricielle, la matrice des corrélations entre les variables et les axes discriminants s’écrit :
$$ C = D_X^{-1} \, (X' W F) \, D_F^{-1} $$

où $D_X$ et $D_F$ sont les matrices diagonales contenant respectivement les normes pondérées des colonnes de $X$ et de $F$.
...
Les coefficients de la matrice $C$ représentent les corrélations pondérées entre les variables initiales et les axes discriminants issus de l’ADPLS. Une valeur absolue élevée (proche de $1$) indique une forte contribution de la variable à la construction de l’axe considéré, tandis qu’une valeur faible (proche de $0$) traduit une influence limitée.

L’analyse de la matrice $C$ constitue une étape fondamentale pour relier les composantes discriminantes aux variables d’origine. Elle permet d’interpréter la signification des axes obtenus en identifiant les variables qui structurent le plus efficacement la séparation entre les groupes.
...
Dans cette dernière étape, nous visualisons les résultats obtenus dans un plan factoriel $(h,m)$ choisi par l'utilisateur. L'objectif comporte trois volets :

- Afficher les individus dans le plan factoriel, colorés selon leur classe.
- Afficher les centres de gravité des $q$ classes sur ce même plan.
- Afficher les variables de $X$ projetées sur le plan choisi et tracer le cercle unité pour faciliter l'interprétation des corrélations.

Pour cela, nous utilisons la fonction `plot_adpls()` qui génère la représentation complète dans le plan $(h, m)$.
...
Cette fonction permet de visualiser simultanément les trois éléments clés de l'ADPLS : la disposition des individus colorés par classe, la position des centres de gravité et la contribution des variables initiales. Le cercle unité facilite l'interprétation des corrélations : une variable dont la flèche atteint ou dépasse le cercle contribue fortemenet à l'axe considéré.
