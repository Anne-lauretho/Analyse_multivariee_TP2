...
Pour chaque rang $h$, la composante discriminante est définie par $f_h = X M u_h$, où $u_h$ désigne le vecteur propre associé à la plus grande valeur propre de la matrice$E_h = \hat{X}' W \hat{X}$ avec $\widehat{X} = \Pi_Y X$ la projection de $X$ sur l’espace engendré par les indicatrices de groupes.
...
Le code renvoie également les vecteurs $u_h$, les composantes discriminantes $F = (f_h)$, les composantes normalisées $\tilde{F}$, ainsi que les quantités intermédiaires $\widehat{X}$, $E$ et le projecteur $\Pi_Y$.
...
Dans cette étape, nous évaluons la qualité des composantes discriminantes obtenues à partir de l’ADPLS. 
...
où $\hat{f}_h = \Pi_Y f_h = \Pi_Y X M u_h$ et $\Pi_Y = Y (Y' W Y)^{-1} Y' W$.
...
On note $F = [f_1, f_2, \ldots, f_H]$ la matrice des composantes discriminantes $(n \times H)$. Chaque individu $i$ a un vecteur de scores factoriels $f_i = (f_{i1}, \ldots, f_{iH})$.
